pipeline {
  ...
  stages {
    ...
    stage('RUN MYSQL') {
      ...
      steps {
        sh "docker run --network ${INPUT_NETWORK_NAME} --name ${INPUT_MYSQL_CONTAINER} -e MYSQL_DATABASE=${INPUT_DB_NAME} -e MYSQL_USER=${INPUT_MYSQL_USER_NAME} -e MYSQL_PASSWORD=${INPUT_MYSQL_USER_PASS} -e MYSQL_RANDOM_ROOT_PASSWORD=yes -d mysql:5.7 --character-set-server=utf8"
